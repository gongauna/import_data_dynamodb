...
async function generateCollectionHouseRecords(houses, country) {
  ...
  readXlsxFile('./data-fraude.xlsx', { schema: schemaLoans, sheet: 'Hoja1'}).then(async (rows) => {
          ...
          PutRequest: {
            Item: {
              ...
              "data": {
                "M": {
                  "text": {
                    "S": `El crédito ${row.loan_id} ha sido asignado a la casa de cobranza Vana Fraudes el día ${assignedAt.toISOString()}.`
                  }
                }
              },
              "loan_id": {
